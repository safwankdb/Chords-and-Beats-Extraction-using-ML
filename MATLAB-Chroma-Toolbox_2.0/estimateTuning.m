...
%
% Description:
% - input is either a mono audio signal or a real valued specgram (magnitude or power)
% - if input is a specgram you have to set parameter.timeinfo and
% parameter.freqinfo (vectors defining the time and freq centers for each specgram bin)
...
if ~inputIsAudioSignal
    if isfield(parameter,'freqinfo')==0
        error('When using a specgram input you have to set parameter.freqinfo');
    end
end
...
if inputIsAudioSignal
    [s,f,t] = specgram(f_input, parameter.fftWindowLength, 22050, parameter.windowFunction(parameter.fftWindowLength), parameter.fftWindowLength/2);
else
    s = f_input;
...
averagedPowerspecgram = sum(s.^2,2);
...
for tu=1:numTunings
    ...
    % build triangular filterbank for magnitude specgram
    specgramFilter = zeros(length(f),1);
    for k=1:length(parameter.pitchRange)        
        ...
        specgramFilter(c:u) = parameter.pitchWeights(k) * linspace(1,0,u-c+1);
        specgramFilter(l:c) = parameter.pitchWeights(k) * linspace(0,1,c-l+1);
    end

    totalPitchEnergyViaSpec(tu) = sum(specgramFilter.^2 .* averagedPowerspecgram);    
end
